...
import {
    ...
    panicPrompt,
    requestWrite,
} from "/home/kdog3682/2024-javascript/nodekit/deno.js"
...
function generateDirectoryIndexFile(dir) {
    const files = readdir(dir, "files")
    const outpath = joinPath(dir, "index.js")
    pop3(files, outpath)

    const mapper = (file) => {
        const s = read(file)
        const defaultExport = match(s, /^export default (\w+)/m)
        const name = removeExtension(tail(file))
        if (defaultExport) {
            return name
        }
        panicPrompt(`
            generateDirectoryIndexFile has very specific requirements
            every file in the given directory: $dir
            must expose export default
            the current file: $file, does not meet this requirement.
        `, {dir, file})
    }

    const t = `import $1 from "./$1.js"`

    const creator = (name) => {
        return templater(t, name)
    }

    const names = files.map(mapper)
    const imports = names.map(creator)
    const exports = moduleExports(names)
    const code = join(imports, exports)
    requestWrite(outpath, code)
}
